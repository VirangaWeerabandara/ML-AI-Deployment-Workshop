...
// ─── Helpers ────────────────────────────────────────────────
function showLoading(el) { el.style.display = 'block'; el.innerHTML = '<h3>Result</h3><div class="loading"><span class="spinner"></span> Processing...</div>'; }
function showError(el, msg) { el.style.display = 'block'; el.innerHTML = `<h3>Result</h3><div class="error-msg">❌ ${msg}<br><small>Make sure the server is running.</small></div>`; }
...
async function checkConnection(url) {
    ...
    try {
        ...
        if (r.ok) { dot.classList.add('connected'); txt.textContent = 'Connected'; return true; }
    } catch(e) {}
    dot.classList.remove('connected'); txt.textContent = 'Disconnected'; return false;
}

// ─── Sentiment Analysis ────────────────────────────────────
const quickTexts = {
    positive: "I absolutely love this product! It's amazing and wonderful!",
    negative: "This is the worst experience I've ever had. Terrible and horrible.",
    neutral: "The meeting is scheduled for 3 PM in the conference room."
};

function quickSentiment(type) { document.getElementById('sentimentText').value = quickTexts[type]; analyzeSentiment(); }
...
async function analyzeSentiment() {
    ...
    try {
        const r = await fetch(url + '/predict', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({text}) });
        const d = await r.json();
        const cls = d.prediction === 'positive' ? 'positive' : d.prediction === 'negative' ? 'negative' : 'neutral';
        const emoji = cls === 'positive' ? '😊' : cls === 'negative' ? '😠' : '😐';
        const color = cls === 'positive' ? 'var(--green)' : cls === 'negative' ? 'var(--red)' : 'var(--yellow)';
...
async function generateLLM() {
    ...
    await checkConnection(url);
    try {
        const r = await fetch(url + '/generate', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({prompt, max_tokens, temperature}) });
        const d = await r.json();
...
async function compareTemps() {
    ...
    showLoading(el);
    try {
        const r = await fetch(url + '/compare-temperatures?prompt=' + encodeURIComponent(prompt) + '&max_tokens=' + max_tokens, {method:'POST'});
        const d = await r.json();
...
async function runBatchTest() {
    ...
    showLoading(el);
    await checkConnection(url);
    const items = Array.from({length: count}, (_, i) => ({text: `Sample review number ${i+1} with great content`}));
    try {
        // Single requests
        const t1 = performance.now();
        for (const item of items) { await fetch(url + '/predict', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(item)}); }
        const singleMs = performance.now() - t1;
        // Batch request
        const t2 = performance.now();
        await fetch(url + '/predict/batch', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(items)});
